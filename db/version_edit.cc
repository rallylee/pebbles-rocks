...
// we don't know this field.
  ...
  void VersionEdit::Clear() {
    ...
    for (size_t i = 0; i < new_guards_.size(); i++) {
      new_guards_[i].clear();
  ...
  bool VersionEdit::EncodeTo(std::string* dst) const {
    ...
    // Encode added guards
    for (size_t k = 0; k < new_guards_.size(); k++) {
...
/*      if (g.number_segments > 0) {
      ...
      for(size_t j = 0; j < g.files.size(); j++) {
        PutVarint32(dst, kFileInsideGuard);
        PutVarint64(dst, g.files[j]);
      }
    }*/
      }
    }
...
/*  for (size_t k = 0; k < cfd->ioptions()->num_levels; k++) {
  for (size_t i = 0; i < sentinel_file_nos_[k].size(); i++) {
    PutVarint32(dst, kNewSentinelFileNo);
    PutVarint32(dst, k);  // level
    PutVarint64(dst, sentinel_file_nos_[k][i]);
  }
}*/

/*  for (size_t k = 0; k < cfd->ioptions()->num_levels; k++) {
  for (size_t i = 0; i < sentinel_files_[k].size(); i++) {
    const FileMetaData& f = sentinel_files_[k][i];
    PutVarint32(dst, kNewSentinelFileNo);
    PutVarint32(dst, k);  // level
